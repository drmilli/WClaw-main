...
const ENSEMBLE_MEMBERS = 40;
const CACHE_TTL_MS = 6 * 60 * 60 * 1000; // 6 hours (ICON updates 4x daily)
...
export async function fetchIconEnsemble(city: CityConfig): Promise<EnsembleForecast> {
  ...
  logger.debug({ city: city.name }, "Fetching ICON ensemble");

  const res = await fetchWithRetry(url.toString());
  if (!res.ok) {
    throw new Error(`Open-Meteo ICON API error: ${res.status} ${res.statusText}`);
  }
  ...
  logger.info({ city: city.name, days: forecast.daily.length }, "ICON forecast cached");
...
function parseEnsembleResponse(city: CityConfig, data: OpenMeteoEnsembleResponse): EnsembleForecast {
  ...
  // Collect all member keys: temperature_2m + temperature_2m_member01..member39
  ...
  if (memberKeys.length < ENSEMBLE_MEMBERS) {
    logger.warn({ city: city.name, members: memberKeys.length }, "Fewer ICON members than expected");
  }
...
export async function fetchAllIconEnsembles(cities: CityConfig[]): Promise<Map<string, EnsembleForecast>> {
  const results = new Map<string, EnsembleForecast>();
  // Sequential with delay to avoid 429 rate limits from Open-Meteo
  for (const city of cities) {
    try {
      const forecast = await fetchIconEnsemble(city);
      results.set(city.slug, forecast);
    } catch (err) {
      logger.error({ city: city.name, err }, "Failed to fetch ICON ensemble");
    }
